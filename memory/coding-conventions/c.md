...
베이스: Linux kernel 코딩 스타일 + C11 표준. c-executor 에이전트는 코드 작성과 자체 리뷰 시 본 문서를 정본으로 참조한다.
...
공개 심볼(헤더에 노출되는 함수, 타입, 매크로)은 **모듈명 접두사**를 갖는다. 모듈이 `hash_map`이면 `hash_map_init`, `hash_map_t`, `HASH_MAP_DEFAULT_CAP` 같은 형식이다.

내부 정적 심볼은 접두사를 짧게 줄여도 된다(예: 모듈 안에서만 쓰는 헬퍼를 `hm_rehash`로). 한 파일 내에서 일관성을 유지한다.

`_t` 접미사는 opaque typedef에만 쓴다. 일반 구조체 typedef로 남용하지 않는다. POSIX 예약 패턴이라는 점도 있고, opaque 표시로서의 의미가 약해지기 때문이다.
...
```c
// 헤더(api): 내부 구조 숨김. 사용자는 typedef로 본다.
typedef struct hash_map hash_map_t;
hash_map_t *hash_map_create(size_t cap);

// 헤더(공개 layout): typedef 하지 않는다. struct 키워드를 사용자도 쓴다.
struct point { int x; int y; };
void point_translate(struct point *p, int dx, int dy);
```

이유는 두 가지다. 먼저 `struct` 키워드의 유무가 가시성을 알린다(가시 layout이냐 opaque이냐). 그리고 Linux kernel 관례와 정합한다.
...
- 탭 들여쓰기. 탭 폭 8 가정. 공백 혼용 금지.
- K&R 중괄호: 함수 정의는 여는 중괄호를 다음 줄에, 그 외(if/for/while)는 같은 줄에.
- 80열 권고. 100열 상한. 긴 식은 인자 단위로 줄바꿈.
- 한 줄짜리 if도 중괄호를 권고한다(Apple `goto fail` 버그 같은 단일 문 함정을 회피).

```c
int hash_map_get(struct hash_map *map, const char *key, void **out_value)
{
	if (!map || !key) {
		return -EINVAL;
	}
...
K&R 분리 함수 정의를 강제한다. **반환형은 별도 줄**에 둔다.
...
```c
// fast-path: 키 길이가 inline 한계 이하면 stack 버퍼만 쓴다.
if (key_len <= HASH_MAP_INLINE_KEY_MAX) { ... }
```

## 8. 리소스 관리: `goto cleanup` 패턴

`malloc`, `fopen`, `pthread_create` 같이 짝이 있는 리소스를 다루는 함수는 단일 cleanup 출구로 정리한다. 중첩 if 회피와 누수 차단이 핵심이다.

```c
int load_config(const char *path, struct config **out_cfg)
{
	int rc = 0;
	FILE *fp = NULL;
...
이유는 두 가지다. 코드 가독성이 의도 신호를 드러내고, 표준 라이브러리 시그니처와의 묵시 변환 경고가 줄어든다. 모든 정수를 `uint32_t` 류로 통일하면 의도 정보가 사라진다.

## 11. 헤더와 모듈 구조

- 헤더 한 개당 모듈 한 개. `module/foo.h` ↔ `module/foo.c`.
- 헤더 가드는 `#pragma once`를 기본으로 한다. 미지원 컴파일러 대응이 필요하면 `MODULE_FOO_H` 형식의 include guard로 대체.
...
- NULL 역참조: 외부에서 받은 포인터는 함수 진입 게이트에서 점검.
